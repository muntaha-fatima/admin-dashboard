    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      if (op.type === "merge_node" && op.path.length === 1 && "markDefs" in op.properties && op.properties._type === editorActor.getSnapshot().context.schema.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
        ...
        if (editor.isTextBlock(targetBlock)) {
          ...
          debug$e("Copying markDefs over to merged block", op), slate.Transforms.setNodes(editor, {
            markDefs: newMarkDefs
          }, {
            at: targetPath,
            voids: !1
...
async function updateValue({
  ...
  if ((!value || value.length === 0) && (debug$4("Value is empty"), slate.Editor.withoutNormalizing(slateEditor, () => {
    ...
    withRemoteChanges(slateEditor, () => {
      withoutPatching(slateEditor, () => {
        ...
        if (!oldBlock && blockValid) {
          const validationValue = [value[currentBlockIndex]], validation = validateValue(validationValue, context.schema, context.keyGenerator);
          debug$4.enabled && debug$4("Validating and inserting new block in the end of the value", currentBlock), validation.valid || validation.resolution?.autoResolve ? slate.Transforms.insertNodes(slateEditor, currentBlock, {
...
function _updateBlock(slateEditor, currentBlock, oldBlock, currentBlockIndex) {
  if (slate.Transforms.setNodes(slateEditor, currentBlock, {
    ...
    currentBlock.children.length < oldBlockChildrenLength && Array.from(Array(oldBlockChildrenLength - currentBlock.children.length)).forEach((_, index) => {
      ...
      childIndex > 0 && (debug$4("Removing child"), slate.Transforms.removeNodes(slateEditor, {
      ...
      if (isChildChanged)
        if (currentBlockChild._key === oldBlockChild?._key) {
          ...
          isSpanNode2 && isTextChanged ? (oldBlockChild.text.length > 0 && slate.deleteText(slateEditor, {
            ...
            at: path
          }), slateEditor.onChange()) : isSpanNode2 || (debug$4("Updating changed inline object child", currentBlockChild), slate.Transforms.setNodes(slateEditor, {
            _key: VOID_CHILD_KEY
          }, {
            at: [...path, 0],
            voids: !0
          }));
        } else oldBlockChild ? (debug$4("Replacing child", currentBlockChild), slate.Transforms.removeNodes(slateEditor, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slate.Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slateEditor.onChange()) : oldBlockChild || (debug$4("Inserting new child", currentBlockChild), slate.Transforms.insertNodes(slateEditor, currentBlockChild, {
          at: [currentBlockIndex, currentBlockChildIndex]
        }), slateEditor.onChange());
    });
  }
...
function RenderSpan(props) {
  const slateEditor = slateReact.useSlateStatic(), editorActor = React.useContext(EditorActorContext), legacySchema = react.useSelector(editorActor, (s) => s.context.getLegacySchema()), spanRef = React.useRef(null), portableTextEditor = usePortableTextEditor(), blockSelected = slateReact.useSelected(), [focused, setFocused] = React.useState(!1), [selected, setSelected] = React.useState(!1), parent = props.children.props.parent, block = parent && slateEditor.isTextBlock(parent) ? parent : void 0, path = React.useMemo(() => block ? [{
    _key: block._key
  }, "children", {
