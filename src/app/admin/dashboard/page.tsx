...
// import ProtectedRoute from "../../component/prducted/page";
...
//           <h2 className="text-2xl font-bold mb-4 text-black text-center">Orders</h2>
//           <div className="overflow-x-auto bg-black shadow-md rounded-lg">
//             <table className="min-w-full divide-y divide-gray-800 text-sm lg:text-base">
...
// import { fetchOrders } from "../../../sanity/lib/order";

// interface Order { _id: string; customerName: string; email: string; totalAmount: number; status: string; }
...
//         <thead><tr><th>Customer</th><th>Email</th><th>Amount</th><th>Status</th></tr></thead>
//         <tbody>
//           {orders.map((order) => (
//             <tr key={order._id}><td>{order.customerName}</td><td>{order.email}</td><td>${order.totalAmount}</td><td>{order.status}</td></tr>
//           ))}
//         </tbody>
//       </table>
//     </div>
...
//   useEffect(() => {
//     const fetchOrders = async () => {
//       const res = await fetch("http://localhost:3001/api/orders"); // GET request
//       const data = await res.json();
